...
unsigned int nTargetSpacing = 1 * 1620; // FidelCastro - 27 minute blocks
...
unsigned int nModifierInterval = 10 * 60; // FidelCastro - time to elapse before new modifier is computed
...
const string strMessageMagic = "FidelCastro Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 1)
        {
            nSubsidy = 166000000 * COIN;
        }
	    else if(pindexBest->nHeight < 512034)
    	{
            nSubsidy = 1953 * COIN;
    	}
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 512034)
    	{
            nSubsidy = 1953 * COIN;
    	}
	    else if(pindexBest->nHeight > 512033)
        {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "FidelCastro", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "No thieves, no traitors, no interventionists! This time the revolution is for real!";
        CTransaction txNew;
        txNew.nTime = 1500136706;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1500136706;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 5791222;
        ...
        assert(block.hashMerkleRoot == uint256("0x65ea1e3a5e2a43c0d94102b5c441e7edba06c8cb124be450047de15aeb6b12c6"));
