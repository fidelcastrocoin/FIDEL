    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->FidelCastroAvailableLabel->setPalette(sample_palette);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "TOTAL(BTC)"<< "FidelCastro(SIZE)" << "BID(BTC)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(BTC)"  << "FidelCastro(SIZE)" << "TOTAL(BTC)");
    /*OrderBook Table Init*/
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(FidelCastro)"<<"TOTAL COST(BTC");
...
void tradingDialog::UpdaterFunction(){
    //FidelCastrost get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=BTC-FidelCastro");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=BTC-FidelCastro&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-FidelCastro&count=100");
      return Response;
}
...
QString tradingDialog::BuyFidelCastro(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=BTC-FidelCastro&quantity=";
...
QString tradingDialog::SellFidelCastro(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=FidelCastro-BTC";
...
QString tradingDialog::GetBalance(QString Currency){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getbalance?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=FidelCastro";
            URL += Currency;
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=FidelCastro-BTC&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->volumet->setText("<b>FidelCastro Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> FidelCastro");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    double FidelCastroSupply = 0;
    double FidelCastroDemand = 0;
    double BtcSupply  = 0;
    double BtcDemand  = 0;
    ...
    foreach (const QJsonValue & value, SellArray)
    {
        ...
        FidelCastroSupply = FidelCastroSupply + y;
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        FidelCastroDemand = FidelCastroDemand + y;
        ...
        ui->FidelCastroSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(FidelCastroSupply,'i',8) + "</span><b> FidelCastro</b>");
        ...
        ui->FidelCastroDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(FidelCastroDemand,'i',8) + "</span><b> FidelCastro</b>");
    ...
    switch (index){
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       QString balance = GetBalance("FidelCastro");
                                       QString str;
                                       QJsonObject ResultObject =  GetResultObjectFromJSONObject(balance);

                                       ui->FidelCastroAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("BTC");
                         ...
                         Response = GetBalance("FidelCastro");

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->FidelCastroBalanceLabel,*ui->FidelCastroAvailableLabel,*ui->FidelCastroPendingLabel, QString::fromUtf8("FidelCastro"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputFidelCastro->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
{
    //calculate amount of BTC that can be gained from selling FidelCastro available balance
    QString responseA = GetBalance("FidelCastro");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailableFidelCastro = ResultObject["Available"].toDouble();

    ui->UnitsInputFidelCastro->setText(str.number(AvailableFidelCastro,'i',8));
}
...
void tradingDialog::on_BuyFidelCastro_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyFidelCastro(Order,Quantity,Rate);
...
void tradingDialog::on_SellFidelCastroBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputFidelCastro->text().toDouble();
    ...
    QString Msg = "Are you sure you want to Sell ";
            Msg += ui->UnitsInputFidelCastro->text();
            Msg += " FidelCastro @ ";
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellFidelCastro(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputFidelCastro_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
